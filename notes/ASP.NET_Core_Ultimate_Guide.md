...
2. ViewBag
是 ViewData 的动态属性封装，更灵活，但不推荐用于复杂数据。
...
4. TempData
用于短生命周期的临时数据传递，通常用于页面跳转时的回传数据。

```C#
TempData["Message"] = "Temporary message";
```

5. 通过 ViewBag 或 ViewData 的属性进行回传
